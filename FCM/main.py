...
def show_images(gray, blur, seg, cont_org, cont_mask):
    ...
    output_img_path = os.path.join(PATH, "output_1.png")
...
def fcm(img):
    Z = img.reshape((-1, 1))
    Z = np.float32(Z)
    fuzz = FCM(image = Z, image_bit = 8, n_clusters = 8, m = 2, epsilon = 0.01, max_iter = 100)
...
def main(path):
    ...
    process(img3=gray, b =130)

    # return (".\FCM\\output\\output_1.png")
    output_img_path = os.path.join(PATH, "output_1.png")
    new_file_path = f"{output_img_path}"
    return (new_file_path)

# if __name__ == '__main__':
#     path = r".\img\1-127.dcm"
#     main(path)